...
/*
vector<int> getCandidateVariables_1(const Trie &trie, const vector<int> &vars, int idx, int left, int i) {
    vector<int> res;
    if (trie.size() == 0 or left < 0) return {};
    ...
    // Vamos hasta la variable que queremos
    for (; i < trie.size() and trie[i].varNumber != vars[idx]; i++);
    if (i == trie.size()) return res;

    vector<int> next_candidates = getCandidateVariables(trie[i].next, vars, idx+1, left, 0);
    vector<int> sibl_candidates = getCandidateVariables(trie, vars, idx+1, left-1, i+1);

    if (next_candidates.size() == 0) return sibl_candidates;
    else if (sibl_candidates.size() == 0) return next_candidates;
    res = getIntersection(next_candidates, sibl_candidates);
